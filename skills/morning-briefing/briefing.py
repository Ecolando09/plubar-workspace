...
def format_discord_embed(weather, crypto, viral, newsletter, apps_status):
    ...
    return {
        "title": "☀️ Good Morning, Landon!",
        "description": f"*{datetime.now().strftime('%A, %B %d, %Y')}*",
        "color": 0x6B5B95,  # Purple
        "fields": [
            {"name": "🌤️ Weather", "value": weather, "inline": False},
            {"name": "🦉 Moonbirds", "value": crypto, "inline": True},
            {"name": "🔥 Viral Posts", "value": viral, "inline": False},
            {"name": "📬 Newsletter", "value": newsletter, "inline": False},
            {"name": "📱 Apps", "value": apps_status, "inline": False}
        ],
        "footer": {"text": "Generated by OpenClaw • 8AM EST"},
...
<body>
    <div class="container">
        <div class="header">
            <h1>☀️ Good Morning, Landon!</h1>
            <p>{datetime.now().strftime('%A, %B %d, %Y')}</p>
        </div>
        <div class="content">
            <div class="section">
                <h2>🌤️ Weather - Spartanburg, SC</h2>
                <div class="weather">
                    {weather}
                </div>
            </div>
            <div class="section">
                <h2>🦉 Moonbirds Ecosystem</h2>
                <div class="crypto">
                    {crypto}
                </div>
            </div>
        ...
        <div class="footer">
            Generated by OpenClaw • {datetime.now().strftime('%I:%M %p EST')}
        </div>
    </div>
</body>
</html>"""
...
def get_weather():
    ...
    try:
        ...
        codes = {0: "☀️ Clear", 1: "⛅ Partly cloudy", 2: "☁️ Cloudy", 3: "🌧️ Rain", 51: "🌧️ Drizzle", 61: "🌧️ Rain", 95: "⛈️ Thunderstorm"}
        cond = codes.get(d["weathercode"][0], "🌤️ Fair")
        return f"**{cond}**\nHigh: {high_f}°F | Low: {low_f}°F\n{'🌧️ ' + str(precip) + 'mm rain' if precip > 0 else '☀️ No rain expected'}\n🌅 {d['sunrise'][0][11:16]} | 🌇 {d['sunset'][0][11:16]}"
...
def get_newsletter():
    ...
    try:
        ...
        return f"**{t}**\n{s}...\n🔗 [Read more](https://theinnermostloop.substack.com)"
    except:
...
def main():
    ...
    html = format_html_email(
        ...
        '<br>'.join([f"• {l}" for l in viral.split('\n') if l.strip()]),
        newsletter.replace('\n', '<br>'),
        apps_status.replace('\n', '<br>')
    )
    ...
    print("="*50)
    print("\n📁 Files saved:")
    print(f"   • {output_dir}/briefing_{today}.md (Markdown)")
    print(f"   • {output_dir}/briefing_{today}.json (Discord Embed)")
    print(f"   • {output_dir}/briefing_{today}.html (Email)")
    print(f"   • {output_dir}/briefing_{today}_voice.txt (TTS Script)")
